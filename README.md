...
```
 [ Fable 5 xhigh ] [ Ctx 12% / 467k ] [ Session 4% ↻ 4h3m · Weekly 27% ↻ 1d22h ] [ ⛏ 28.8M ] [ 🎀 ULTRA ]
user@host:~/some/long/project/dir
```

- **model + effort** — from statusline JSON (`.model.display_name`, `.effort.level`)
- **Ctx** — share of the auto-compact budget in use, then the budget itself
  (the token count where auto-compact fires). The percentage is floored and
  clamped, so `100%` shows exactly when compaction fires and never before
  (`99%` means within 1%, about 4.7k tokens, of it). Plain dim text, no tier
  colors: context is informational, not a limit. The count behind the
  percentage is what Claude Code's compaction trigger actually compares, not
  just the last API usage: last response tokens (input + cache-create +
  ...
  denominator is the full window and the badge is marked `∅ compact`. Before
  the first API response (fresh session, or right after a compaction) the
  count is the pending estimate alone, as the trigger sees it. Note
  `/context` reports a lower token count
  than the badge's basis: it shows the last usage without output tokens or
  pending tool results, and its percentage is of the full window, not the
  compact budget.
